...
public interface pthread_once$__init_routine {
    ...
    static MemorySegment allocate(pthread_once$__init_routine fi, MemorySession session) {
        return RuntimeHelper.upcallStub(pthread_once$__init_routine.class, fi, constants$74.pthread_once$__init_routine$FUNC, session);
    }
    static pthread_once$__init_routine ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return () -> {
            try {
                constants$74.pthread_once$__init_routine$MH.invokeExact((Addressable)symbol);
            } catch (Throwable ex$) {
