...
public interface pthread_atfork$__prepare {
    ...
    static MemorySegment allocate(pthread_atfork$__prepare fi, MemorySession session) {
        return RuntimeHelper.upcallStub(pthread_atfork$__prepare.class, fi, constants$87.pthread_atfork$__prepare$FUNC, session);
    }
    static pthread_atfork$__prepare ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return () -> {
            try {
                constants$87.pthread_atfork$__prepare$MH.invokeExact((Addressable)symbol);
            } catch (Throwable ex$) {
