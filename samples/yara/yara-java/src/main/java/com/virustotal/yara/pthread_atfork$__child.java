...
public interface pthread_atfork$__child {
    ...
    static MemorySegment allocate(pthread_atfork$__child fi, MemorySession session) {
        return RuntimeHelper.upcallStub(pthread_atfork$__child.class, fi, constants$87.pthread_atfork$__child$FUNC, session);
    }
    static pthread_atfork$__child ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return () -> {
            try {
                constants$87.pthread_atfork$__child$MH.invokeExact((Addressable)symbol);
            } catch (Throwable ex$) {
